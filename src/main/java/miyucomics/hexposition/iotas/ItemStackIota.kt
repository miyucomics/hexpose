package miyucomics.hexposition.iotas
...
class ItemStackIota(stack: ItemStack) : Iota(TYPE, stack) {
	...
	companion object {
		const val TAG_STACK_ID: String = "hexposition:stack_id"
		const val TAG_STACK_COUNT: String = "hexposition:stack_count"
		const val TAG_STACK_NBT: String = "hexposition:stack_tag"
		...
		var TYPE: IotaType<ItemStackIota> = object : IotaType<ItemStackIota>() {
			...
			override fun display(tag: NbtElement): Text {
				...
				if (stack.isEmpty)
					return Text.translatable("hexposition.item_stack.null").formatted(Formatting.GRAY)
				return Text.literal("[item:" + (tag as NbtCompound).getString(TAG_STACK_ID) + "]").append(Text.translatable("hexposition.item_stack.format", Text.empty().append(stack.name).formatted(stack.rarity.formatting), stack.count))
			}
